...
<svg style="display: none;">
    ...
    <section class="page-content">
        <section class="search-and-user">
                    <h1 style="font-size: 35px; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;"><i>FACTURAS Y ENVÍOS</i></h1>
...
<script>
    ...
    function changeProfilePic(event) {
        ...
        <hr>
        <br>
        <h1 style="font-size: 25px; color: green;">Envíos</h1>
        <br>
        <hr>
        <?php
...
// Define la consulta SQL para seleccionar campos de la tabla envios.
$envios_sql = "SELECT id_envio, id_pedido, nombre_completo, direccion, ciudad, telefono FROM envios";
// Ejecuta la consulta SQL y almacena el resultado en la variable $envios_result.
$envios_result = $conexion->query($envios_sql);

// Comprueba si la consulta ha devuelto alguna fila.
if ($envios_result->num_rows > 0) {
    // Si hay filas, genera una tabla HTML para mostrar los datos.
    echo "<table class='styled-table'>";
    echo "<thead><tr><th>ID Envío</th><th>ID Pedido</th><th>Nombre Completo</th><th>Dirección</th><th>Ciudad</th><th>Teléfono</th><th>Acciones</th></tr></thead>";
    echo "<tbody>";

    // Itera a través de las filas devueltas por la consulta.
    while ($envios_row = $envios_result->fetch_assoc()) {
        // Genera una fila de la tabla por cada envío.
        echo "<tr>";
        echo "<td>" . $envios_row["id_envio"] . "</td>";
        echo "<td>" . $envios_row["id_pedido"] . "</td>";
        echo "<td>" . $envios_row["nombre_completo"] . "</td>";
        echo "<td>" . $envios_row["direccion"] . "</td>";
        echo "<td>" . $envios_row["ciudad"] . "</td>";
        echo "<td>" . $envios_row["telefono"] . "</td>";
        echo "<td>
        <form method='post' action='../app/vistas/productos/eliminar_envio.php' style='display:inline;' onsubmit='return confirmarEliminacion1();'>
                    <input type='hidden' name='id_envio' value='" . $envios_row["id_envio"] . "'>
    ...
    // Si no hay filas, muestra un mensaje indicando que no hay envíos disponibles.
...
<!-- JavaScript -->
<script>
// Función para mostrar una confirmación antes de eliminar un envío.
function confirmarEliminacion1() {
    return confirm('¿Estás seguro de que deseas eliminar este envío?');
}
</script>

<h1 style="font-size: 25px; color: green;">Facturas</h1>
<br>
<hr>
<?php 
// Define la consulta SQL para seleccionar campos de la tabla facturación.
$facturacion_sql = "SELECT id_factura, id_pedido, id_cliente, fecha_emision, monto_total FROM facturacion";
// Ejecuta la consulta SQL y almacena el resultado en la variable $facturacion_result.
$facturacion_result = $conexion->query($facturacion_sql);

// Comprueba si la consulta ha devuelto alguna fila.
if ($facturacion_result->num_rows > 0) {
    // Si hay filas, genera una tabla HTML para mostrar los datos.
    echo "<table class='styled-table'>";
    echo "<thead><tr><th>ID Factura</th><th>ID Pedido</th><th>ID Cliente</th><th>Fecha Emisión</th><th>Monto Total</th><th>Acciones</th></tr></thead>";
    echo "<tbody>";

    // Itera a través de las filas devueltas por la consulta.
    while ($facturacion_row = $facturacion_result->fetch_assoc()) {
        // Genera una fila de la tabla por cada factura.
        echo "<tr>";
        echo "<td>" . $facturacion_row["id_factura"] . "</td>";
        echo "<td>" . $facturacion_row["id_pedido"] . "</td>";
        echo "<td>" . $facturacion_row["id_cliente"] . "</td>";
        echo "<td>" . $facturacion_row["fecha_emision"] . "</td>";
        echo "<td>$" . $facturacion_row["monto_total"] . "</td>";
        echo "<td>
        <form method='post' action='../app/vistas/productos/eliminar_factu.php' style='display:inline;' onsubmit='return confirmarEliminacion();'>
                    <input type='hidden' name='id_factura' value='" . $facturacion_row["id_factura"] . "'>
                    <center><button type='submit' class='btn btn-danger' title='Eliminar'>
                        <i class='fas fa-trash'></i>
                    </button></center>
                </form>";
    ...
    // Si no hay filas, muestra un mensaje indicando que no hay facturas disponibles.
...
<!-- JavaScript -->
<script>
// Función para mostrar una confirmación antes de eliminar una factura.
