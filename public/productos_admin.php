...
<table id="datatable_users" class="table table-striped" style="text-align: center;">
    <thead>
        ...
        <tr>
            ...
            <input id="file-upload" type="file" name="imagen" required/></td>
...
if ($result->num_rows > 0) {
    // Si hay filas, genera una tabla HTML para mostrar los productos.
    ...
    while ($row = $result->fetch_assoc()) {
        // Genera una fila de la tabla por cada producto.
        echo "<tr>";
        echo "<td>" . $row["id_producto"] . "</td>";
        echo "<td>" . $row["nombre"] . "</td>";
        echo "<td>$" . $row["precio"] . "</td>";
        // Muestra la imagen del producto en la tabla.
        echo "<td><img src='" . $row["imagen"] . "' class='table-img'></td>";
        // Añade botones para editar y eliminar el producto.
        echo "<td>
                <a href='?edit=" . $row["id_producto"] . "' title='Editar' class='btn btn-sm btn-primary'><i class='fa fa-pencil'></i></a>
                <button class='btn btn-sm btn-danger' title='Eliminar' onclick='eliminarProducto(" . $row["id_producto"] . ")'><i class='fa fa-trash'></i></button>
              </td>";
        echo "</tr>";
    }
...
// Comprueba si se ha pasado el parámetro 'edit' en la URL.
if (isset($_GET['edit'])) {
    // Obtiene el id del producto desde el parámetro 'edit' de la URL.
    $id_producto = $_GET['edit'];
    // Define la consulta SQL para seleccionar el producto con el id proporcionado.
...
if (isset($_POST['update'])) {
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] == 0) {
        ...
        if ($tipoImagen == "jpg" || $tipoImagen == "jpeg" || $tipoImagen == "png") {
            ...
            $ruta_imagen = $carpeta . basename($Nombre_imagen);

            // Mueve el archivo de imagen subido a la ruta definida.
            if (move_uploaded_file($imagen, $ruta_imagen)) {
                // Define la consulta SQL para actualizar el producto con el nuevo nombre, precio e imagen.
                $sql = "UPDATE productos SET nombre='$nombre', precio='$precio', imagen='$ruta_imagen' WHERE id_producto='$id_producto'";
            } else {
                // Si la imagen no se pudo mover, muestra un mensaje de error.
        ...
        // Si no se ha subido una nueva imagen, define la consulta SQL para actualizar solo el nombre y el precio del producto.
        $sql = "UPDATE productos SET nombre='$nombre', precio='$precio' WHERE id_producto='$id_producto'";
    }

    // Ejecuta la consulta y comprueba si se realizó correctamente.
    if ($conexion->query($sql) === TRUE) {
        // Si la actualización es exitosa, establece un mensaje de éxito.
        $mensaje = "Producto actualizado, recarga la página...";
        ...
        // Establece el tipo de mensaje como error.
        $tipo_mensaje = "error";
    }
}
    ...
    <form method="post" action="" enctype="multipart/form-data">
        ...
        <thead>       
            <tr>
                <th class="centered">Nombre del producto</th>
                <th class="centered">Precio</th>
                <th class="centered" style="float: left;margin-left: 4.4%;">Imagen del producto</th>
            </tr>
        </thead> 
        <tbody id="tableBody_users">
        <tr>
        ...
        <button name="update" id="boton" class="btn btn-sm btn-primary">
                <span>Actualizar producto</span></button>
        <input type="hidden" name="id_producto" value="<?php echo $row['id_producto']; ?>">
        <?php endif; ?>
        <?php if (isset($mensaje)): ?>
    <div id="alerta" class="alert <?php echo $tipo_mensaje; ?>">
        <?php echo $mensaje; ?>
    </div>
<?php endif; ?>
    </form>
