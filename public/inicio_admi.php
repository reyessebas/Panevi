...
<svg style="display: none;">
    ...
    <section class="page-content">
        <section class="search-and-user">
                    <h1 style="font-size: 35px; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;"><i>INICIO</i></h1>
...
<script>
    ...
    function changeProfilePic(event) {
        ...
        <hr>
        <br>
        <h2 style="font-size: 32px; color: green;">Bienvenido, <?php echo htmlspecialchars($_SESSION['nombre']); ?></h2><br>
<hr>
<?php
// Incluye el archivo de configuración que contiene los datos de conexión a la base de datos.
include("../app/config/config.php");

// Define la consulta SQL para calcular la suma del monto total de todas las facturas.
$total_ventas_sql = "SELECT SUM(monto_total) AS total_ventas FROM facturacion";
// Ejecuta la consulta SQL y almacena el resultado en la variable $total_ventas_result.
$total_ventas_result = $conexion->query($total_ventas_sql);
// Recupera el valor de 'total_ventas' del resultado de la consulta.
$total_ventas = $total_ventas_result->fetch_assoc()['total_ventas'];

// Define la consulta SQL para contar el número de pedidos con estado 'pendiente'.
$pedidos_pendientes_sql = "SELECT COUNT(*) AS pedidos_pendientes FROM pedidos WHERE estado = 'pendiente'";
// Ejecuta la consulta SQL y almacena el resultado en la variable $pedidos_pendientes_result.
$pedidos_pendientes_result = $conexion->query($pedidos_pendientes_sql);
// Recupera el valor de 'pedidos_pendientes' del resultado de la consulta.
$pedidos_pendientes = $pedidos_pendientes_result->fetch_assoc()['pedidos_pendientes'];

// Define la consulta SQL para contar el número de pedidos con estado 'finalizado'.
$pedidos_finalizados_sql = "SELECT COUNT(*) AS pedidos_finalizados FROM pedidos WHERE estado = 'finalizado'";
// Ejecuta la consulta SQL y almacena el resultado en la variable $pedidos_finalizados_result.
$pedidos_finalizados_result = $conexion->query($pedidos_finalizados_sql);
// Recupera el valor de 'pedidos_finalizados' del resultado de la consulta.
$pedidos_finalizados = $pedidos_finalizados_result->fetch_assoc()['pedidos_finalizados'];

// Define la consulta SQL para contar el número de clientes.
$cantidad_clientes_sql = "SELECT COUNT(*) AS cantidad_clientes FROM clientes";
// Ejecuta la consulta SQL y almacena el resultado en la variable $cantidad_clientes_result.
$cantidad_clientes_result = $conexion->query($cantidad_clientes_sql);
// Recupera el valor de 'cantidad_clientes' del resultado de la consulta.
$cantidad_clientes = $cantidad_clientes_result->fetch_assoc()['cantidad_clientes'];

// Define la consulta SQL para contar el número de productos.
$cantidad_productos_sql = "SELECT COUNT(*) AS cantidad_productos FROM productos";
// Ejecuta la consulta SQL y almacena el resultado en la variable $cantidad_productos_result.
$cantidad_productos_result = $conexion->query($cantidad_productos_sql);
// Recupera el valor de 'cantidad_productos' del resultado de la consulta.
$cantidad_productos = $cantidad_productos_result->fetch_assoc()['cantidad_productos'];
?>


<div style="display: flex; justify-content: space-between; margin-top: 20px;">
<div style="flex: 1; margin-right: 20px;">
    <h3 style="font-size: 23px; color: green;">Indicadores Clave</h3>
    <br>
    <ul style="list-style-type: none; padding: 0;">
        <li style="font-size: 20px; font-weight: bold">Total de Ventas: $<?php echo number_format($total_ventas, 2); ?> Pesos</li>
        <br>
        <li style="font-size: 20px; font-weight: bold">Pedidos Pendientes: <?php echo $pedidos_pendientes; ?></li>
        <br>
        <li style="font-size: 20px; font-weight: bold">Pedidos Finalizados: <?php echo $pedidos_finalizados; ?></li>
        <br>
        <li style="font-size: 20px; font-weight: bold">Cantidad de Clientes: <?php echo $cantidad_clientes; ?></li>
        <br>
        <li style="font-size: 20px; font-weight: bold">Cantidad de Productos: <?php echo $cantidad_productos; ?></li>
    </ul>
    
    <form action="reporte_pdf.php" method="post">
    <button type="submit" id="boton" class="btn btn-primary" style="background-color: #275d1e; border-color: white; color: white; padding: 10px 20px; font-size: 16px; border-radius: 20px; display: inline-flex; align-items: center; transition: background-color 0.3s ease, transform 0.3s ease;"
   ...
   <i class="fas fa-chart-line" style="margin-right: 10px;"></i> Ver Reporte
</button>
    </form>
</div>

    <div style="flex: 1;">
        <h3 style="font-size: 23px; color: green;">Accesos rápidos</h3>
        <br>
        <a href="clien_admi.php" style="font-size: 20px; font-weight: bold"><b>Consultar Clientes</b></a><br><br>
        <a href="productos_admin.php" style="font-size: 20px; font-weight: bold"><b>Gestionar productos</b></a><br><br>
        <a href="pedidos_admi.php" style="font-size: 20px; font-weight: bold"><b>Gestionar pedidos</b></a><br><br>
        <a href="factu_envios.php" style="font-size: 20px; font-weight: bold"><b>Consultar facturas y envíos</b></a><br><br>
        <a href="confi_admi.php" style="font-size: 20px; font-weight: bold"><b>Cambiar usuario y contraseña</b></a><br><br>
    </div>
</div>
