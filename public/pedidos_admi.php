...
<svg style="display: none;">
    ...
    <section class="page-content">
        <section class="search-and-user">
                    <h1 style="font-size: 35px; font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif;"><i>PEDIDOS</i></h1>
...
<script>
    ...
    function changeProfilePic(event) {
        ...
        <hr>
        <br>
        <h1 style="font-size: 25px; color: green;">Actualizar Pedidos</h1>
        <br>
        <hr>
        <?php
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Itera a través de las fechas de envío enviadas por POST.
    foreach ($_POST['fecha_envio'] as $id_pedido => $fecha_envio) {
        // Recupera las fechas de pedido y entrega, y el estado del pedido desde el formulario.
        $fecha_pedido = $_POST['fecha_pedido'][$id_pedido]; // Nuevo campo
        $fecha_entrega = $_POST['fecha_entrega'][$id_pedido];
        $estado = $_POST['estado'][$id_pedido];

        // Define la consulta SQL para actualizar el pedido con las nuevas fechas y estado.
        $sql = "UPDATE pedidos SET fecha_pedido='$fecha_pedido', fecha_envio='$fecha_envio', fecha_entrega='$fecha_entrega', estado='$estado' WHERE id_pedido='$id_pedido'";
        // Ejecuta la consulta.
        $conexion->query($sql);
    }
    // Establece un mensaje de éxito si se guardaron los cambios.
    $mensaje = "Cambios guardados exitosamente.";
    // Establece el tipo de mensaje como éxito.
    $tipo_mensaje = "success";
}

// Define la consulta SQL para seleccionar todos los campos de la tabla pedidos.
$tab_sql = "SELECT id_pedido, id_cliente, fecha_pedido, fecha_envio, fecha_entrega, estado, total FROM pedidos";
...
if ($result->num_rows > 0) {
    // Si hay resultados, inicia un formulario HTML con método POST.
    echo "<form method='post' action=''>";
    // Genera la tabla HTML con clases y estilos para formatearla.
    echo "<table class='styled-table'>";
    echo "<thead><tr><th>ID Pedido</th><th>ID Cliente</th><th>Fecha Pedido</th><th>Fecha Envío</th><th>Fecha Entrega</th><th>Estado</th><th>Total</th><th>Acciones</th></tr></thead>";
    echo "<tbody>";
    // Itera a través de cada fila del resultado de la consulta.
    while ($row = $result->fetch_assoc()) {
        // Genera una fila de la tabla por cada registro de pedido.
        echo "<tr>";
        echo "<td>" . $row["id_pedido"] . "</td>";
        echo "<td>" . $row["id_cliente"] . "</td>";
        // Genera un campo de entrada de fecha para la fecha del pedido.
        echo "<td><input type='date' name='fecha_pedido[" . $row["id_pedido"] . "]' value='" . $row["fecha_pedido"] . "' class='form-control'></td>"; // Nuevo campo
        // Genera un campo de entrada de fecha para la fecha de envío.
        echo "<td><input type='date' name='fecha_envio[" . $row["id_pedido"] . "]' value='" . $row["fecha_envio"] . "' class='form-control'></td>";
        // Genera un campo de entrada de fecha para la fecha de entrega.
        echo "<td><input type='date' name='fecha_entrega[" . $row["id_pedido"] . "]' value='" . $row["fecha_entrega"] . "' class='form-control'></td>";
        // Genera un campo de selección para el estado del pedido con opciones predefinidas.
        echo "<td>
                <select name='estado[" . $row["id_pedido"] . "]' class='form-control estado-select'>
                    ...
                    <option value='Devuelto' " . ($row["estado"] == 'Devuelto' ? 'selected' : '') . ">Devuelto</option>
                    <option value='Finalizado' " . ($row["estado"] == 'Finalizado' ? 'selected' : '') . ">Finalizado</option>
                </select>
              </td>";
        // Muestra el total del pedido.
        echo "<td>$" . $row["total"] . "</td>";
        // Genera botones para guardar cambios y eliminar el pedido.
        echo "<td><center><button type='submit' title='Guardar' class='btn btn-sm btn-success'><i class='fa fa-save'></i></button></center>
        <form method='post' action='../app/vistas/productos/eliminar_pedido.php' style='display:inline;' onsubmit='return confirmarEliminacion1();'>
                    <input type='hidden' name='id_pedido' value='" . $row["id_pedido"] . "'>
                    <center><button type='submit' class='btn btn-danger' title='Eliminar'>
    ...
    echo "</tbody>";
    echo "</table>";
    // Cierra el formulario HTML.
    echo "</form>";
} else {
    // Si no hay resultados, muestra un mensaje indicando que no hay pedidos disponibles.
    echo "No hay pedidos disponibles.";
}

// Comprueba si hay un mensaje y lo muestra en una alerta HTML.
if ($mensaje) {
    echo "<div class='alert alert-$tipo_mensaje'>$mensaje</div>";
}
?>
<script>
// Función para confirmar la eliminación de un pedido.
function confirmarEliminacion1() {
...
<h1 style="font-size: 25px; color: green;">Detalle de los Pedidos</h1>
<br>
<hr>
<?php
// Define la consulta SQL para seleccionar los detalles de los pedidos.
$detalle_sql = "SELECT id_detalle_pedido, id_pedido, id_producto, precio_unitario, cantidad, subtotal FROM detalle_pedido";
// Ejecuta la consulta SQL y almacena el resultado en la variable $detalle_result.
$detalle_result = $conexion->query($detalle_sql);

// Comprueba si la consulta ha devuelto alguna fila.
if ($detalle_result->num_rows > 0) {
    // Si hay filas, genera una tabla HTML para mostrar los detalles de los pedidos.
    echo "<table class='styled-table'>";
    echo "<thead><tr><th>ID Detalle Pedido</th><th>ID Pedido</th><th>ID Producto</th><th>Precio Unitario</th><th>Cantidad</th><th>Subtotal</th><th>Acciones</th></tr></thead>";
    echo "<tbody>";

    // Itera a través de las filas devueltas por la consulta.
    while ($detalle_row = $detalle_result->fetch_assoc()) {
        // Genera una fila de la tabla por cada detalle de pedido.
        $id_detalle_pedido = $detalle_row["id_detalle_pedido"];
        echo "<tr>";
        echo "<td>" . $detalle_row["id_detalle_pedido"] . "</td>";
        echo "<td>" . $detalle_row["id_pedido"] . "</td>";
        echo "<td>" . $detalle_row["id_producto"] . "</td>";
        echo "<td>$" . $detalle_row["precio_unitario"] . "</td>";
        echo "<td>" . $detalle_row["cantidad"] . "</td>";
        echo "<td>$" . $detalle_row["subtotal"] . "</td>";
        echo "<td>
        <form method='post' action='../app/vistas/productos/eliminar_detalle.php' style='display:inline;' onsubmit='return confirmarEliminacion();'>
                    <input type='hidden' name='id_detalle_pedido' value='" . $detalle_row["id_detalle_pedido"] . "'>
                    <center><button type='submit' class='btn btn-danger' title='Eliminar'>
                        <i class='fas fa-trash'></i>
                    </button></center>
                </form>";
    ...
    // Si no hay filas, muestra un mensaje indicando que no hay detalles de pedidos disponibles.
...
<!-- JavaScript -->
<script>
// Función para mostrar una confirmación antes de eliminar un detalle de pedido.
function confirmarEliminacion(id) {
    return confirm('¿Estás seguro de que deseas eliminar este detalle de pedido?');
}
</script>
